...
export default class ProductService {
    ...
    async createProduct(product){
        ...
        if(!productAdded) throw new InternalServerError("Product not created.");
    ...
    async deleteProduct(id){
        ...
        if(!productDelete) throw new NotFoundError("Product not found.");
        return productDelete;
    }

    async updateProductStock(id, quantity){
        const productUpdate = await this.productRepository.updateStock(id, quantity);
        if (productUpdate.nModified === 0) throw new ConflictError("Not enough stock.");
    }
